...
const CanvaTemplates = () => {
 ...
 // Public user view: show Buyer PDF generator form
  return (
    <Box sx={{ py: 4, maxWidth: 600, mx: 'auto', pl: 0, ml: 0 }}>
      ...
      <form onSubmit={handlePublicSubmit} style={{ marginLeft: 0, paddingLeft: 0 }}>
        <Box display="flex" flexDirection="column" gap={2} sx={{ pl: 0, ml: 0 }}>
          <FormControl fullWidth sx={{ pl: 0, ml: 0 }}>
          ...
          {pdfType === 'invite-suite' && (
            <>
              <TextField label="RSVP Canva Link" value={rsvpCanvaLink} onChange={e => setRsvpCanvaLink(e.target.value)} fullWidth sx={{ pl: 0, ml: 0 }} />
              <TextField label="Detail Card Canva Link" value={detailCardCanvaLink} onChange={e => setDetailCardCanvaLink(e.target.value)} fullWidth sx={{ pl: 0, ml: 0 }} />
            </>
          )}
          {formError && <Alert severity="error">{formError}</Alert>}
          <Button type="submit" variant="contained" color="primary" disabled={formLoading} fullWidth sx={{ pl: 0, ml: 0 }}>
